...
[![arduino-library-badge](https://www.ardu-badge.com/badge/AsyncESP32_Ethernet_Manager.svg?)](https://www.ardu-badge.com/AsyncESP32_Ethernet_Manager)
[![GitHub release](https://img.shields.io/github/release/khoih-prog/AsyncESP32_Ethernet_Manager.svg)](https://github.com/khoih-prog/AsyncESP32_Ethernet_Manager/releases)
[![GitHub](https://img.shields.io/github/license/mashape/apistatus.svg)](https://github.com/khoih-prog/AsyncESP32_Ethernet_Manager/blob/main/LICENSE)
[![contributions welcome](https://img.shields.io/badge/contributions-welcome-brightgreen.svg?style=flat)](#Contributing)
[![GitHub issues](https://img.shields.io/github/issues/khoih-prog/AsyncESP32_Ethernet_Manager.svg)](http://github.com/khoih-prog/AsyncESP32_Ethernet_Manager/issues)
...
* [Why do we need this AsyncESP32_Ethernet_Manager library](#why-do-we-need-this-async-AsyncESP32_Ethernet_Manager-library)
  * [Features](#features)
...
### Why do we need this Async [AsyncESP32_Ethernet_Manager library](https://github.com/khoih-prog/AsyncESP32_Ethernet_Manager)

#### Features

This is an `ESP32 + LwIP LwIP W5500 / ENC28J60` Credentials and Connection Manager with fallback Web ConfigPortal. This Library is used for configuring ESP32 Ethernet Static / DHCP and Credentials at runtime. You can specify static DNS servers, personalized HostName and CORS feature.
...
<p align="center">
    <img src="https://github.com/khoih-prog/AsyncESP32_Ethernet_Manager/raw/main/Images/W5500.png">
</p>

<p align="center">
    <img src="https://github.com/khoih-prog/AsyncESP32_Ethernet_Manager/raw/main/Images/W5500_small.png">
</p> 

##### ENC28J60

<p align="center">
    <img src="https://github.com/khoih-prog/AsyncESP32_Ethernet_Manager/raw/main/Images/ENC28J60.png">
</p>
...
## Prerequisites
 ...
 8. [`WebServer_ESP32_ENC library v1.5.1+`](https://github.com/khoih-prog/WebServer_ESP32_ENC) if necessary to use ESP32 boards using LwIP ENC28J60 Ethernet. To install, check [![arduino-library-badge](https://www.ardu-badge.com/badge/WebServer_ESP32_ENC.svg?)](https://www.ardu-badge.com/WebServer_ESP32_ENC)
...
The best and easiest way is to use `Arduino Library Manager`. Search for `AsyncESP32_Ethernet_Manager`, then select / install the latest version. You can also use this link [![arduino-library-badge](https://www.ardu-badge.com/badge/AsyncESP32_Ethernet_Manager.svg?)](https://www.ardu-badge.com/AsyncESP32_Ethernet_Manager) for more detailed instructions.

### Manual Install

1. Navigate to [AsyncESP32_Ethernet_Manager](https://github.com/khoih-prog/AsyncESP32_Ethernet_Manager) page.
2. Download the latest release `AsyncESP32_Ethernet_Manager-main.zip`.
3. Extract the zip file to `AsyncESP32_Ethernet_Manager-main` directory 
4. Copy the whole `AsyncESP32_Ethernet_Manager-main` folder to Arduino libraries' directory such as `~/Arduino/libraries/`.

### VS Code & PlatformIO:

1. Install [VS Code](https://code.visualstudio.com/)
2. Install [PlatformIO](https://platformio.org/platformio-ide)
3. Install [**AsyncESP32_Ethernet_Manager** library](https://registry.platformio.org/libraries/khoih-prog/AsyncESP32_Ethernet_Manager) by using [Library Manager](https://registry.platformio.org/libraries/khoih-prog/AsyncESP32_Ethernet_Manager/installation). Search for **AsyncESP32_Ethernet_Manager** in [Platform.io Author's Libraries](https://platformio.org/lib/search?query=author:%22Khoi%20Hoang%22)
...
Thanks to [Roshan](https://github.com/solroshan) to report the issue in [Error esp_littlefs.c 'utime_p'](https://github.com/khoih-prog/AsyncESP32_Ethernet_Manager/issues/28) 
...
```cpp
#include <AsyncESP32_Ethernet_Manager.hpp>               //https://github.com/khoih-prog/AsyncESP32_Ethernet_Manager
```

in many files. But be sure to use the following `#include <AsyncESP32_Ethernet_Manager.h>` **in just 1 `.h`, `.cpp` or `.ino` file**, which must **not be included in any other file**, to avoid `Multiple Definitions` Linker Error

```cpp
// To be included only in main(), .ino with setup() to avoid `Multiple Definitions` Linker Error
#include <AsyncESP32_Ethernet_Manager.h>          //https://github.com/khoih-prog/AsyncESP32_Ethernet_Manager
```

Check [Async_ConfigOnDoubleReset_Multi](examples/Async_ConfigOnDoubleReset_Multi) for an example how and where to do so.

Have a look at the discussion in [Different behaviour using the src_cpp or src_h lib #80](https://github.com/khoih-prog/AsyncESP32_Ethernet_Manager/discussions/80)
...
// Must be placed before #include <AsyncESP32_Ethernet_Manager.h>
...
///////////////////////////////////////////
/******************************************
   // Defined in AsyncESP32_Ethernet_Manager.hpp
  typedef struct
  {
...
These illustrating steps is based on the example [Async_ConfigOnSwitchFS](https://github.com/khoih-prog/AsyncESP32_Ethernet_Manager/tree/main/examples/Async_ConfigOnSwitchFS)
...
The example [Async_ConfigOnSwitchFS](https://github.com/khoih-prog/AsyncESP32_Ethernet_Manager/tree/main/examples/Async_ConfigOnSwitchFS) will open the CP whenever a SW press is detected in `loop()`. So the code to add `dynamic variables` will be there, just after the CP `AsyncESP32_Ethernet_Manager` class initialization to create `AsyncESP32_Ethernet_Manager` object.
...
Adding those `ESPAsync_EMParameter` objects created in Step 2 using the function `addParameter()` of object `AsyncESP32_Ethernet_Manager`
...
After users select `Save`, the CP `AsyncESP32_Ethernet_Manager` object will save the user input data into related `ESPAsync_EMParameter` objects.
...
<p align="center">
    <img src="https://github.com/khoih-prog/AsyncESP32_Ethernet_Manager/raw/main/Images/Saved.png">
</p>
...
void loop()
{
  ...
  if ((digitalRead(TRIGGER_PIN) == LOW) || (digitalRead(TRIGGER_PIN2) == LOW))
  {
    ...
    AsyncESP32_Ethernet_Manager AsyncESP32_Ethernet_manager(&webServer, NULL, "ConfigOnSwitch");
#else
    AsyncDNSServer dnsServer;

    AsyncESP32_Ethernet_Manager AsyncESP32_Ethernet_manager(&webServer, &dnsServer, "ConfigOnSwitch");
#endif
    ...
    AsyncESP32_Ethernet_manager.setSTAStaticIPConfig(stationIP, gatewayIP, netMask);
#endif
#endif

#if USING_CORS_FEATURE
    AsyncESP32_Ethernet_manager.setCORSHeader("Your Access-Control-Allow-Origin");
#endif
    ...
    if (loadConfigData())
    {
      AsyncESP32_Ethernet_manager.setConfigPortalTimeout(
    ...
    else
    {
      ...
      AsyncESP32_Ethernet_manager.setConfigPortalTimeout(0);
    ...
    if (!AsyncESP32_Ethernet_manager.startConfigPortal())
    ...
    const char * TZ_Result = AsyncESP32_Ethernet_manager.getTZ(Ethconfig.TZ_Name);
    ...
    AsyncESP32_Ethernet_manager.getSTAStaticIPConfig(EthSTA_IPconfig);
...
Many applications need configuration parameters like `MQTT host and port`, [Blynk](http://www.blynk.cc) or [emoncms](http://emoncms.org) tokens, etc. While it is possible to use [`AsyncESP32_Ethernet_Manager`](https://github.com/khoih-prog/AsyncESP32_Ethernet_Manager) to collect additional parameters, it is better to read these parameters from a web service once [`AsyncESP32_Ethernet_Manager`](https://github.com/khoih-prog/AsyncESP32_Ethernet_Manager) has been used to connect to the Internet.

To capture other parameters with [`AsyncESP32_Ethernet_Manager`](https://github.com/khoih-prog/AsyncESP32_Ethernet_Manager) is a little bit more complicated than all the other features. This requires adding custom HTML to your form. 

If you want to do it with [`AsyncESP32_Ethernet_Manager`](https://github.com/khoih-prog/AsyncESP32_Ethernet_Manager) see the example [Async_ConfigOnSwitchFS](examples/Async_ConfigOnSwitchFS)
...
https://github.com/khoih-prog/AsyncESP32_Ethernet_Manager/blob/9d426c29c71bb2ec4d8fba8aa61fef1544dde740/examples/Async_ConfigOnSwitch/Async_ConfigOnSwitch.ino#L26-L1011
...
#### 1. Async_ConfigOnDoubleReset using LittleFS on ESP32_DEV with ESP32_ENC28J60

This is terminal debug output when running [Async_ConfigOnDoubleReset](examples/Async_ConfigOnDoubleReset) on  **ESP32_ENC28J60**.


```
Starting Async_ConfigOnDoubleReset using LittleFS on ESP32_DEV with ESP32_ENC28J60
AsyncESP32_Ethernet_Manager v1.0.0
...
[EM] dns1IP = 192.168.2.1 , dns2IP = 8.8.8.8
ETH Connected
ETH MAC: DE:AD:BE:EF:FE:11, IPv4: 192.168.2.233
FULL_DUPLEX, 10Mbps
...
Local Date/Time: Sun Dec 11 23:03:36 2022
Stop doubleResetDetecting
Saving config file...
Saving config file OK
Local Date/Time: Sun Dec 11 23:04:35 2022
Local Date/Time: Sun Dec 11 23:05:35 2022
```
...
Submit issues to: [AsyncESP32_Ethernet_Manager issues](https://github.com/khoih-prog/AsyncESP32_Ethernet_Manager/issues)
...
- The library is licensed under [MIT](https://github.com/khoih-prog/AsyncESP32_Ethernet_Manager/blob/main/LICENSE)
